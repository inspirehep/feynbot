...
async def export_queries(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=queries_ir_{
                start_date.date()
            }_{end_date.date()}.csv"
        },
    )
...
async def query_os(
    ...
    """Send the query OpenSearch endpoint and return its response with highlights."""
...
async def export_feedback(
    ...
    if csv:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=search_feedback_{
                    start_date.date()
                }_{end_date.date()}.csv"
            },
        )
